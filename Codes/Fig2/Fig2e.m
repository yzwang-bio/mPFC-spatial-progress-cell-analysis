...
subplot(1,2,1)
b = bar(means_before, 'FaceColor', [0.7 0.7 0.9]);
hold on;
errorbar(1:3, means_before, sems_before, 'k.', 'LineWidth', 1.5, 'CapSize', 10);
hold off;
xticklabels({'Time from trial start','Distance to goal','Norm. distance (spatial progress)'});
ylabel('Response jitter');
set(gca, 'FontSize', 12, 'Box', 'off');
set(gcf, 'Color', 'w');
title(categ{1})
subplot(1,2,2)
b = bar(means_after, 'FaceColor', [0.7 0.7 0.9]);
hold on;
